...
const features = [
  ...
  { icon: Mic, title: "Voice Input", desc: "Answer naturally using your microphone. Powered by Whisper." },
    ...
    <div className="min-h-screen bg-night-950">
      ...
      <main className="pt-32 pb-24 px-6 max-w-7xl mx-auto">
        <div className="text-center max-w-3xl mx-auto mb-20 animate-fade-in">
          <div className="inline-flex items-center gap-2 bg-accent/10 border border-accent/20 text-accent text-sm px-4 py-1.5 rounded-full mb-8">
            <Zap size={12} /> Powered by Groq & LLaMA 3.3
          </div>
          <h1 className="font-display text-6xl font-bold leading-tight mb-6">
            Ace your next<br />
            <span className="bg-gradient-to-r from-accent to-accent-light bg-clip-text text-transparent">interview</span> with AI
          </h1>
          ...
          <div className="flex items-center justify-center gap-4">
            <Link href="/setup"><Button size="lg">Start Practicing</Button></Link>
            <Link href="/dashboard"><Button size="lg" variant="outline">View Dashboard</Button></Link>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {features.map((f) => (
            <div key={f.title} className="glass rounded-2xl p-6 hover:border-accent/20 transition-all duration-300 group">
